...
require_once('class/class.eventtype.php');
...
$action = 'EventType';
$Event_obj = new MasterEventTypeClass();
$ajax = new PHPLiveX();
$ajax->AjaxifyObjects(array("Event_obj")); 
?>
<!DOCTYPE html>
<html lang="en">
<head>
   ...
   switch($index)
	{
		case 'new_type':
		if($submit=='Save Info')
		$Event_obj->New_Type('server');	
		else
		$Event_obj->New_Type('local');
		break;
		case 'edit_source_Type':
		if($submited=='Edit Info')
		$Event_obj->Edit_Type('server',$sourceId);	
		else
		$Event_obj->Edit_Type('local',$sourceId);
	 break;
	default :
		echo $Event_obj->EventValuesManagement();
		break;
	}
	?>
	</div>

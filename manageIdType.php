...
require_once('class/class.masterIDTypes.php');
...
$action = 'IDType';
$IDType_obj = new MasterIDTypeClass();
$ajax = new PHPLiveX();
$ajax->AjaxifyObjects(array("IDType_obj")); 
?>
<!DOCTYPE html>
<html lang="en">
<head>
   ...
   switch($index)
	{
		case 'new_type':
		if($submit=='Save Info')
		$IDType_obj->New_IDtype('server');	
		else
		$IDType_obj->New_IDtype('local');
		break;
		case 'edit_IdType':
		if($submited=='Edit Info')
		$IDType_obj->Edit_IDType('server',$idTypeID);	
		else
		$IDType_obj->Edit_IDType('local',$idTypeID);
	 	break;
	default :
		echo $IDType_obj->IDTypeValuesManagement();
		break;
	}
	?>
	</div>

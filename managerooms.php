...
require_once('class/class.masterRooms.php');
...
$action = 'Rooms';
$Rooms_obj = new MasterRoomsClass();
$ajax = new PHPLiveX();
$ajax->AjaxifyObjects(array("Rooms_obj")); 
?>
<!DOCTYPE html>
<html lang="en">
<head>
   ...
   switch($index)
	{
		case 'new_type':
		if($submit=='Save Info')
		$Rooms_obj->New_Room('server');	
		else
		$Rooms_obj->New_Room('local');
		break;
		case 'edit_room':
		if($submited=='Edit Info')
		$Rooms_obj->Edit_Room('server',$roomId);	
		else
		$Rooms_obj->Edit_Room('local',$roomId);
	 	break;
	default :
		echo $Rooms_obj->RoomsValuesManagement();
		break;
	}
	?>
	</div>

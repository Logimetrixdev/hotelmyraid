...
class KotMenu
{
		...
		switch($case_finder)
		{
			...
			<form  enctype="multipart/form-data" action="" class="form-horizontal group-border-dashed" method="post" name="addNewMenuHeader">
               ...
               <div class="block-flat">
                <a href="kot_menus.php" class="btn btn-primary btn-flat" style="float:right;">View F&B Menu</a>
					...
					/* End Validation  */
						...
						if($return){
							...
							$insert_sql_array['header_name'] = ucwords($this->header_name);
							 $this->db->insert(HMS_KOT_MENU_HEADER,$insert_sql_array);
							
							$_SESSION['msg'] = 'Menu header has been Successfully Added';
							
							?>
							<script type="text/javascript">
								window.location = "kot_menus.php?index=new_item"
							</script>
							<?php
							exit();
		...
		switch($case_finder)
		{
              ...
              <div class="block-flat">
                 <a href="kot_menus.php" class="btn btn-primary btn-flat" style="float:right;">View F&B Menu</a>
					...
					<div class="content">
							...
							<?php
							$sql_header="select * from ".HMS_KOT_MENU_HEADER." where deleted='1' and status='1'";
                            ...
                            <div class="form-group">
                            <div class="col-sm-8">
                            <label>Pax Cost</label> 
                            <input  type="text" name="price" data-parsley-type="digits"  class="form-control"  required/>
                            </div>
                            </div>
			...
			extract($_POST);
						...
						if($this->Form->ValidField($item,'empty','Please Enter Item Name')==false)
							$return =false;
							if($this->Form->ValidField($code,'empty','Please enter item code')==false)
							$return =false;
						if($this->Form->ValidField($price,'empty','Please Enter Item price')==false)
							$return =false;
						/* End Validation  */
						...
						if($return){
							...
							$insert_sql_array['item_code'] = strtoupper($this->code);
							$insert_sql_array['per_pack_cost'] = $this->price;
							$this->db->insert(HMS_KOT_MENU,$insert_sql_array);
							$_SESSION['msg'] = 'Menu item is added in menu header successfully';
							?>
							<script type="text/javascript">
								window.location = "kot_menus.php"
							</script>
							<?php
							exit();
    ...
    function EditItems($case_finder,$item_id)
	{
		...
		{
			case 'local':
			     $sql_item = "select * from ".HMS_KOT_MENU." where id='".$item_id."'";
              ...
              <div class="block-flat">
                 <div class="header">							
					...
					<div class="content">
							...
							<div class="col-sm-8">
							<label>Item Code</label> 
                            <input  type="text" name="code" class="form-control" value="<?php echo $row_item['item_code'];?>"  required/>
                           </div>
							</div>
							...
							<div class="col-sm-8">
							<label>Pax Cost</label> 
                            <input  type="text" name="price"  data-parsley-type="digits" class="form-control" value="<?php echo $row_item['per_pack_cost'];?>"  required/>
                           </div>
							</div>
			...
			extract($_POST);
						...
						if($return){
							...
							$update_sql_array['item_code'] = strtoupper($this->code);
							$update_sql_array['per_pack_cost'] = $this->price;
						$this->db->update(HMS_KOT_MENU,$update_sql_array,'id',$item_id);
							$_SESSION['msg'] = 'Menu item is updated in menu header successfully';
							?>
							<script type="text/javascript">
								window.location = "kot_menus.php?index=editItem&itemId=<?php echo $item_id?>";
							</script>
							<?php
							exit();
	...
	function EditHeader($case_finder,$MenuHeaderId)
	{
		...
		{
			case 'local':
				            $sql_item = "select * from ".HMS_KOT_MENU_HEADER." where id='".$MenuHeaderId."'";
							$result_item = $this->db->query($sql_item,__FILE__,__LINE__);
							$row_item = $this->db->fetch_array($result_item);
			...
			extract($_POST);
					...
					/* End Validation  */
						...
						if($return){
							...
							$this->db->update(HMS_KOT_MENU_HEADER,$update_sql_array,'id',$MenuHeaderId);
							
							$_SESSION['msg'] = 'Menu header has been Successfully updated';
							
							?>
							<script type="text/javascript">
								window.location = "kot_menus.php?index=editMenuHeader&headerId=<?php echo $MenuHeaderId?>";
							</script>
							<?php
							exit();
	...
	function AllKOTMenuList()
	{
		...
		?>
				...
				<div class="col-md-12">
					<div class="block-flat">
                      <a href="kot_menus.php?index=new_header" class="btn btn-primary btn-flat" style="float:right;">Add New Menu Header</a>
                    <a href="kot_menus.php?index=new_item" class="btn btn-primary btn-flat" style="float:right;">Add New Item In Menu</a>
						<div class="header">
                        	<h4>F&B Menu</h4>
                       </div>
						<div class="content">
								...
								<table class="table table-bordered" id="datatable" >
									<thead>
										<tr>
											...
											<th width="25%"><b>Menu Headers</b></th>
											<th width="35%"><b>Items (ITEM CODE)</b></th>
				...
				if($cnt>0)
				{
										...
										<tr class="odd gradeX">
                                           ...
                                           <?php
											...
											if($cntval>0)
											{
												...
												{
													?>
                                                    <a href="kot_menus.php?index=editItem&itemId=<?php echo $row_bitem['id'];?>" class="fancybox fancybox.iframe">
                                                    <img src="images/edit.png" style="height:20px; margin-right:5px; cursor:pointer;" title="Edit This Item"></a>
                                                    <img src="images/trash.png" style="height:20px; margin-right:10px; cursor:pointer;" title="Remove This Item" onclick="javascript: if(confirm('Do u want to delete this item from this header?')) { kot_obj.deleteitem('<?php echo $row_bitem['id'];?>',{}) };">
                                                    <?php
							echo '<b>'.$row_bitem['item'].' ['.$row_bitem['item_code'].'] '.$row_bitem['per_pack_cost'].' Rs /- </b>';
													echo '<br />';
												}
												?>
                                                </p>
						...
						<li><a href="kot_menus.php?index=editMenuHeader&headerId=<?php echo $row['id'];?>" class="fancybox fancybox.iframe" title="Edit">Edit</a></li>
                        <li class="divider"></li><li> <a  href="javascript: void(0);" title="Remove Header" onclick="javascript: if(confirm('If you remove this header it will remove all items as well?')) { kot_obj.deleteheader('<?php echo $row['id'];?>',{}) };" >Remove</a></li></ul></div></td>
										</tr>
										<?php 
							$x++;
							}
	...
	function deleteitem($id)
	{
			...
			$this->db->update(HMS_KOT_MENU,$update_array,'id',$id);
	...
	function deleteheader($id)
	{
			...
			$update_array = array();
			$update_array['deleted'] = 0 ;
			$this->db->update(HMS_KOT_MENU_HEADER,$update_array,'id',$id);
			
			$sql_bitem="select * from ".HMS_KOT_MENU." where header_id='".$id."'";
			$result_bitem = $this->db->query($sql_bitem,__FILE__,__LINE__);
			while($row_bitem = $this->db->fetch_array($result_bitem))
			{
				$update_array = array();
				$update_array['deleted'] = 0 ;
				$this->db->update(HMS_KOT_MENU,$update_array,'id',$row_bitem['id']);
			}

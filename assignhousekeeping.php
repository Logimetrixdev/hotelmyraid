...
$action = 'AllUser';
$housekeep_obj = new HousekeepingClass();
$ajax = new PHPLiveX();
$ajax->AjaxifyObjects(array("housekeep_obj")); 
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<div class="container-fluid" id="pcont">
   ...
   switch($index)
	{
		case 'msg':
		//$housekeep_obj->Housekeeping_Assignment();
	 	break;
		case 'release':
		
		if($submit=='Clear Work')
        $housekeep_obj->ReleaseRoom('server',$room_id,$hk_id);	
        else
        $housekeep_obj->ReleaseRoom('local',$room_id,$hk_id);
		break;
		case 'assign':
		if($submit=='Assign Work')
        $housekeep_obj->AssignUserForHouskeeping('server',$room_id);	
        else
        $housekeep_obj->AssignUserForHouskeeping('local',$room_id);
		break;
	default :

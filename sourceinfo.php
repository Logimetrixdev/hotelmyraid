...
require_once('class/class.SourceInfo.php');
...
$action = 'SourceInfo';
$SourceInfo_obj = new MasterSourceInfoClass();
$ajax = new PHPLiveX();
$ajax->AjaxifyObjects(array("SourceInfo_obj")); 
?>
<!DOCTYPE html>
<html lang="en">
<head>
   ...
   switch($index)
	{
		case 'new_type':
		if($submit=='Save Info')
		$SourceInfo_obj->New_Type('server');	
		else
		$SourceInfo_obj->New_Type('local');
		break;
		case 'edit_source_Type':
		if($submited=='Edit Info')
		$SourceInfo_obj->Edit_Type('server',$sourceId);	
		else
		$SourceInfo_obj->Edit_Type('local',$sourceId);
	 break;
	default :
		echo $SourceInfo_obj->SourceInfoValuesManagement();
		break;
	}
	?>
	</div>

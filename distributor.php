...
require_once('class/class.distributor.php');
...
$action = 'Distributor';
$distributor_obj = new DistributorClass();
$ajax = new PHPLiveX();
$ajax->AjaxifyObjects(array("distributor_obj")); 
?>
<!DOCTYPE html>
<html lang="en">
<head>
   ...
   switch($index)
	{
		case 'new_type':
		if($submit=='Save Info')
		$distributor_obj->New_Type('server');	
		else
		$distributor_obj->New_Type('local');
		break;
		case 'edit_source_Type':
		if($submited=='Edit Info')
		$distributor_obj->Edit_Type('server',$sourceId);	
		else
		$distributor_obj->Edit_Type('local',$sourceId);
	 break;
	default :
		echo $distributor_obj->DistributorManagement();
		break;
	}
	?>
	</div>

...
require_once('class/class.banquet.php');
...
$action = 'Banquet';
$Banquet_obj = new MasterBanquetClass();
$ajax = new PHPLiveX();
   ...
   switch($index)
	{
		case 'new_type':
		if($submit=='Save Info')
		$Banquet_obj->New_Type('server');	
		else
		$Banquet_obj->New_Type('local');
		break;
		case 'edit_source_Type':
		if($submited=='Edit Info')
		$Banquet_obj->Edit_Type('server',$sourceId);	
		else
		$Banquet_obj->Edit_Type('local',$sourceId);
	 break;
	default :
		echo $Banquet_obj->BanquetValuesManagement();
		break;
	}
	?>
	</div>
